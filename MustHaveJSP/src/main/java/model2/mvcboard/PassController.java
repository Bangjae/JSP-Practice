...
public class PassController extends HttpServlet
{
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse resp)
		throws ServletException, IOException
		{
			/*
				...
				방법1 : request내장객체 이용한다. => req.getParameter(파라미터명)
			...
			throws ServletException, IOException
	{
		...
		boolean confirmed = dao.cofirmPassword(pass, idx);
		dao.close();
		...
		if(confirmed)
		{
			if(mode.equals("edit"))
			{
				/*
				 	...
				 	수정완료 할 때까지 유지하고, 수정이 완료되면 제거 할 것이다.
				 	여기에서 저장한 패스워드를 수정을 위한 update 쿼리문에 
				 	where절에 사용할 예정이다.
				*/
				...
				/*
					수정 페이지로 이동한다. 만약 아래 URL패턴을 이용해서 패스워드검증없이
					진입하였다면 세션에 저장된 패스워드가 없을것이므로 수정처리가
					되지않도록 처리할 것이다.
				*/
