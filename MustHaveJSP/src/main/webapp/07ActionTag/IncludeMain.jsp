...
// page영역과 request영역에 각각 속성을 저정한다.
...
<body>
	...
	<%--@ incloude file="<%=outerPath10uterPage1%>" --%>
	<!-- 
		지시어를 통한 인클루드는 포함한 파일의 소스를 그대로의 현재 문서에
		포함시킨 후 컴파일을 진행한다. 따라서 하나의 페이지라 생각하면 된다
		그러므로 newVal1은 정상적으로 출력된다.
	 -->
	...
	<!-- 
		include 지시어와 액션 태그의 차이점
			...
			액션태그
				-jsp소스 먼저 실행한 후 실행된 결과를 포함시킨다.
				-따라서 기존페이지에서 선언한 변수는 포함시킨 페이지에서 
				사용할 수 없다.
				-서로 다른페이지를 의미하므로 page영역은 공유하지 않는다
